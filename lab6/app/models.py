...
from sqlalchemy import String, ForeignKey, DateTime, Text, Integer, MetaData,select
...
class User(Base, UserMixin):
    ...
    def can_review(self,course_id):
        user_reviews=db.session.query(Review).filter(Review.course_id==course_id).filter(Review.user_id==self.id).count()
        if user_reviews==0:
            return True
        else:
            return False
